...
function agentLookupResult(result){
  if(result.verified&&result.agent){const a=result.agent,date=a.connected_at?new Date(a.connected_at).toLocaleDateString(state.lang==='ru'?'ru-RU':'en-GB'):'—';return `<article class="verified-agent-card"><header><i>${icon('check')}</i><div><span>✓ VERIFIED AGENT</span><h3>${esc(a.cashdesk_name||'Cashdesk')}</h3></div></header><dl><dt>Agent ID</dt><dd>${esc(a.agent_id)}</dd><dt>${state.lang==='ru'?'Страна':'Country'}</dt><dd>${esc(a.country||'—')}</dd><dt>${state.lang==='ru'?'Город':'City'}</dt><dd>${esc(a.city||'—')}</dd><dt>${state.lang==='ru'?'Статус':'Status'}</dt><dd>${state.lang==='ru'?'Подтверждён':'Verified'}</dd><dt>${state.lang==='ru'?'Подключён':'Connected'}</dt><dd>${date}</dd></dl></article>`}
...
document.addEventListener('click',async e=>{
  ...
  if(editAgent){state.managerAgentEdit=state.managerApps.find(application=>String(application.id)===String(editAgent.dataset.editAgent))||null;return go('manager_edit_agent')}
...
document.addEventListener('input',e=>{if(e.target.matches('[data-manager-agent-search]')){state.managerAgentQuery=e.target.value;render();const search=content.querySelector('[data-manager-agent-search]');if(search){search.focus();search.setSelectionRange(search.value.length,search.value.length)}return}if(e.target.closest('[data-form="application"]')){state.form={...state.form,[e.target.name]:e.target.value};sessionStorage.setItem('agent-form',JSON.stringify(state.form))}});
...
document.addEventListener('submit',async e=>{
  ...
  try{
    ...
    if(type==='manager-add-agent'||type==='manager-agent-edit'){
      const telegramId=(data.telegram_id||'').trim();const body={telegram_id:telegramId?Number(telegramId):null,telegram_username:(data.telegram_username||'').trim(),name:(data.name||'').trim(),cashdesk_name:(data.cashdesk_name||'').trim(),cashdesk_geo:(data.cashdesk_geo||'').trim(),country:(data.country||'').trim(),phone:(data.phone||'').trim(),email:(data.email||'').trim()};const endpoint=type==='manager-add-agent'?'/api/manager/agents':`/api/manager/agents/${form.dataset.agentId}`;const response=await api(endpoint,{method:type==='manager-add-agent'?'POST':'PUT',body:JSON.stringify(body)});if(!response.ok)throw new Error(await readError(response));const list=await api('/api/manager/applications');state.managerApps=list.ok?await list.json():state.managerApps;state.managerAgentEdit=null;note(type==='manager-add-agent'?'Агент добавлен и подтверждён':'Данные агента обновлены');return go('manager');
    }

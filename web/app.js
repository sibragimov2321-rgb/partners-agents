...
document.addEventListener('submit',async e=>{
  ...
  try{
    ...
    if(type==='manager-add-agent'||type==='manager-agent-edit'){
      const telegramId=(data.telegram_id||'').trim();const body={telegram_id:telegramId?Number(telegramId):null,telegram_username:(data.telegram_username||'').trim(),name:(data.name||'').trim(),cashdesk_name:(data.cashdesk_name||'').trim(),country:(data.country||'').trim(),phone:(data.phone||'').trim(),email:(data.email||'').trim()};const endpoint=type==='manager-add-agent'?'/api/manager/agents':`/api/manager/agents/${form.dataset.agentId}`;const response=await api(endpoint,{method:type==='manager-add-agent'?'POST':'PUT',body:JSON.stringify(body)});if(!response.ok)throw new Error(await readError(response));const list=await api('/api/manager/applications');state.managerApps=list.ok?await list.json():state.managerApps;state.managerAgentEdit=null;note(type==='manager-add-agent'?'Агент добавлен и подтверждён':'Данные агента обновлены');return go('manager');
    }

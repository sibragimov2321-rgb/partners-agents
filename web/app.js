...
function showHome(){screen.hidden=true;home.hidden=false;home.innerHTML=items.map(([icon,title,key])=>`<button class="btn ${key==='agents'?'primary':''}" onclick="openScreen('${key}')">${icon} ${title}</button>`).join('');}
function openScreen(key){
  home.hidden=true; screen.hidden=false;
  const title=items.find(x=>x[2]===key)?.[1]||''; let body='';

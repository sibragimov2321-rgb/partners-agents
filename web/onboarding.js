...
export function renderManager(applications = [], filter = 'all', query = '') {
  ...
  const shown = normalisedQuery ? byStatus.filter(application => [application.name, application.telegram_username, application.telegram_id, application.cashdesk_name, application.agent_id].filter(Boolean).some(value => String(value).toLowerCase().replace(/^@/, '').includes(normalisedQuery))) : byStatus;

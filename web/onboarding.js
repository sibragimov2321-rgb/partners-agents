...
function managerAgentForm(formType, agent, title, subtitle) {
  ...
  return `<div class="view manager-view">${pageHead(title, subtitle)}<form class="flow-card" data-form="${formType}"${attrs}><p class="form-help">ФИО и ФГ доступны только менеджерам. Обычным пользователям при проверке отображается только название кассы.</p><label class="field"><span>Telegram ID</span><input name="telegram_id" inputmode="numeric" pattern="[0-9]+" placeholder="123456789" value="${e(telegramId)}"></label><label class="field"><span>Username Telegram</span><input name="telegram_username" maxlength="32" placeholder="@username" value="${value('telegram_username')}"></label><label class="field"><span>Имя и фамилия *</span><input name="name" required maxlength="160" placeholder="Имя Фамилия" value="${value('name')}"></label><label class="field"><span>Название кассы *</span><input name="cashdesk_name" required maxlength="160" placeholder="Например: Central Cashdesk" value="${value('cashdesk_name')}"></label><label class="field"><span>ФГ</span><input name="cashdesk_fg" maxlength="160" placeholder="Укажите ФГ кассы" value="${value('cashdesk_fg')}"></label><label class="field"><span>Страна *</span><input name="country" required maxlength="100" placeholder="Например: Tajikistan" value="${value('country')}"></label><label class="field"><span>Телефон *</span><input name="phone" required type="tel" placeholder="+992 900 000 000" value="${value('phone')}"></label><label class="field"><span>Email *</span><input name="email" required type="email" placeholder="name@example.com" value="${value('email')}"></label><div class="flow-actions">${action('Назад','type="button" data-nav="manager"','secondary')}${action(button,'type="submit"')}</div></form></div>`;
}

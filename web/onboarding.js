...
export function renderManager(applications = [], filter = 'all') {
  ...
  return `<div class="view manager-view">${pageHead('Кабинет менеджера', `${applications.length} заявок`)}${action('➕ Добавить агента','data-nav="manager-add-agent"')}<div class="manager-filter">${filters.map(([value,label]) => `<button class="${filter === value ? 'active' : ''}" data-manager-filter="${value}">${label}</button>`).join('')}</div>${cards}</div>`;
}

export function renderManagerAddAgent() {
  return `<div class="view manager-view">${pageHead('Добавить агента', 'Создание подтверждённого профиля')}<form class="flow-card" data-form="manager-add-agent"><p class="form-help">Только менеджер может создать агента. Проверьте Telegram ID перед сохранением.</p><label class="field"><span>Telegram ID *</span><input name="telegram_id" required inputmode="numeric" pattern="[0-9]+" placeholder="123456789"></label><label class="field"><span>Username Telegram</span><input name="telegram_username" maxlength="32" placeholder="@username"></label><label class="field"><span>Имя и фамилия *</span><input name="name" required maxlength="160" placeholder="Имя Фамилия"></label><label class="field"><span>Страна *</span><input name="country" required maxlength="100" placeholder="Например: Tajikistan"></label><label class="field"><span>Телефон *</span><input name="phone" required type="tel" placeholder="+992 900 000 000"></label><label class="field"><span>Email *</span><input name="email" required type="email" placeholder="name@example.com"></label><div class="flow-actions">${action('Назад','type="button" data-nav="manager"','secondary')}${action('Добавить агента','type="submit"')}</div></form></div>`;
}
